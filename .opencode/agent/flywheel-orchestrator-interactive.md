...
Otherwise identical: own `runs/<problem>/<ts>/flywheel-state.json` **and `runs/<problem>/<ts>/learnings.md` (append-only learnings loop: append reviewer Learning/Do-not-retry lines every iteration, consolidate Confirmed/Contradicted/Open + baseline diagnosis every 3 iterations or on plateau, inject into each next `sandbox-executor` prompt, log `nudge` summaries via `shared/observe.py`)**, confirm resources once at run start (`df -h /`, `vm_stat`/`free`, `nproc` vs `constraints`, abort/downscale + log on drift), dispatch executor/reviewer/researcher/planner/executor (parallel light variants only; heavy serially, one job → one log), use `nohup` + `while pgrep ... sleep 10`, respect `models:` overrides, log every `Q:`/`A:` alongside `Decision:`/`Rationale:` blocks **via `shared/observe.py append` (dual-writes `events.jsonl` + `decisions.md`; never hand-append)**. Goals immutable: never edit `problems/<name>/problem.yaml` `goal`/`gates`/thresholds — learnings nudge future steps only.
