...
**Goals immutable:** never edit `problems/<p>/problem.yaml` `goal`/`gates`/thresholds. Honor `plan.md` Do-not-retry. When picking the winner, record which carried-forward learning paid off (margins + learning id) for the orchestrator's `--phase flywheel --event decision` log.
...
9. Provenance: the orchestrator logs `decision` events via `shared/observe.py append --phase flywheel` for winner-pick (gate margins) and `--phase done` on success, so `events.jsonl` answers what was done and why.
