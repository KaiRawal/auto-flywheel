...
**Goals immutable:** never propose or apply edits to `problems/<p>/problem.yaml` `goal`/`gates`/thresholds. You surface hypotheses that nudge future *steps* only.

Inputs: all `runs/<p>/<ts>/logs/*.md` + `runs/<p>/<ts>/learnings.md` + `runs/<p>/<ts>/decisions.md` + variant `metrics.json` files + `problems/<p>/problem.yaml` + `shared/gate-contract.md` + any `references` configured per-problem.
...
Provenance: return candidates; the orchestrator logs one `decision` event via `shared/observe.py append --phase research --event decision` with the chosen/rejected rationale and gate delta.
