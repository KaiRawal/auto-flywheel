...
`runs/<problem>/<ts>/events.jsonl` is the machine-readable audit trail.
`runs/<problem>/<ts>/decisions.md` is the human render, dual-written from the
same `shared/observe.py:append_event` call. Both are orchestrator-owned.
...
.venv/bin/python shared/observe.py init --run-dir runs/toy-tabular/<ts> --problem toy-tabular

# log anything (phase + gate deltas only — no prompts/tool traces)
.venv/bin/python shared/observe.py append --run-dir runs/toy-tabular/<ts> \
...
.venv/bin/python shared/observe.py query runs/toy-tabular/<ts> --failed-only
.venv/bin/python shared/observe.py query runs/toy-tabular/<ts> --phase research
.venv/bin/python shared/observe.py query runs/toy-tabular/<ts> --event decision --gate fidelity
jq -c 'select(.gates.fidelity.pass==false)' runs/toy-tabular/<ts>/events.jsonl
```
...
Full learning text lives in `runs/<p>/<ts>/logs/NN.md` + run-scoped
`runs/<p>/<ts>/learnings.md` (orchestrator-owned, append-only). Events carry
