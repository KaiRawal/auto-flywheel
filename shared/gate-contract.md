...
summaries via `shared/observe.py append --event nudge|decision`.
...
`shared/event-schema.md`), which dual-writes:

- `runs/<p>/<ts>/events.jsonl` — one JSON object per line (`phase`, `agent`, `event`, `gates: {<gate>: {pass, margin}}`)
- `runs/<p>/<ts>/decisions.md` — human render, one block per event:
...
`runs/<problem>/<ts>/manifest.json` or `artifacts/manifest.json` records `{sha256, shapes}` per artifact plus `{python, numpy, sklearn, ...}` versions — same pattern as `timpara/opencode-academic-research`'s Material Passport.
